...
$sql = " select SUBSTRING(vs_date,1,4) as vs_year, SUM(vs_count) as cnt 
           from {$gb4['visit_sum_table']}
          where vs_date between '$fr_date' and '$to_date' and vs_blog_id='{$current['id']}' 
          group by vs_year
...
if (count($arr)) {
    foreach ($arr as $key=>$value) {
        ...
        <tr class='list$list ht center'>
            <td><a href='./visit_month.php?mb_id=$mb_id&fr_date=$key-01-01&to_date=$key-12-31' class=tt>$key</a></td>
